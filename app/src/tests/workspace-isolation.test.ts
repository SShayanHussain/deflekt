...
import { GET as GetSources, POST as PostSource } from "../app/api/workspaces/[workspaceId]/sources/route";
import { DELETE as DeleteSource } from "../app/api/workspaces/[workspaceId]/sources/[sourceId]/route";
...
describe("Cross-tenant isolation tests", () => {
  ...
  // Sources API Isolation Tests

  it("should block user from getting sources if not a member", async () => {
    mockDbForIsolation(false);
    
    const req = createRequest("GET", "http://localhost/api/workspaces/ws-1/sources");
    const res = await GetSources(req, { params: Promise.resolve({ workspaceId: "ws-1" }) });
    const json = await res.json();
    
    expect(res.status).toBe(403);
  });

  it("should block user from uploading sources if not a member", async () => {
    mockDbForIsolation(false);
    
    const req = createRequest("POST", "http://localhost/api/workspaces/ws-1/sources", { file: "test" });
    const res = await PostSource(req, { params: Promise.resolve({ workspaceId: "ws-1" }) });
    const json = await res.json();
    
    expect(res.status).toBe(403);
  });

  it("should block user from deleting sources if not a member", async () => {
    mockDbForIsolation(false);
    
    const req = createRequest("DELETE", "http://localhost/api/workspaces/ws-1/sources/doc-1");
    const res = await DeleteSource(req, { params: Promise.resolve({ workspaceId: "ws-1", sourceId: "doc-1" }) });
    const json = await res.json();
